...
fn register_share_builtins(vm: &mut VirtualMachine) {
    ...
    vm.register_foreign_function("Share.get_commitment", |ctx| {
        if ctx.args.len() < 2 {
            return Err("Share.get_commitment expects 2 arguments: share, index".to_string());
        }
        let (_, share_data) =
    ...
    vm.register_foreign_function("Share.mul_field", |ctx| {
        if ctx.args.len() < 2 {
            return Err("Share.mul_field expects 2 arguments: share, field_bytes".to_string());
        }
...
fn share_add(ctx: ForeignFunctionContext) -> Result<Value, String> {
    ...
    let result_data = ctx
        .vm_state
        .secret_share_add(ty1, data1.as_bytes(), data2.as_bytes())?;
...
fn share_sub(ctx: ForeignFunctionContext) -> Result<Value, String> {
    ...
    let result_data = ctx
        .vm_state
        .secret_share_sub(ty1, data1.as_bytes(), data2.as_bytes())?;
...
fn share_add_scalar(ctx: ForeignFunctionContext) -> Result<Value, String> {
    ...
    let result_data = ctx
        .vm_state
        .secret_share_add_scalar(ty, data.as_bytes(), scalar)?;
...
fn share_mul_scalar(ctx: ForeignFunctionContext) -> Result<Value, String> {
    ...
    let result_data = ctx
        .vm_state
        .secret_share_mul_scalar(ty, data.as_bytes(), scalar)?;
...
fn share_batch_open(ctx: ForeignFunctionContext) -> Result<Value, String> {
    ...
    let shares: Vec<Vec<u8>> = share_data
        .iter()
        .map(|(_, d)| d.as_bytes().to_vec())
        .collect();
...
mod tests {
    ...
    fn test_share_object_creation() {
        ...
        let id = share_object::create_share_object(
            &mut store,
            share_type,
    ...
    fn test_is_share_object() {
        ...
        let share_id =
            share_object::create_share_object(&mut store, share_type, ShareData::Opaque(data), 0);
