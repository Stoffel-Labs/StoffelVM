...
impl CompiledBinary {
    ...
    fn add_constant_if_new(&mut self, value: &Value, constant_map: &mut HashMap<Value, usize>) -> usize {
    ...
    /// * `constant_map` - A map from values to their indices in the constant pool
    fn add_function_from_vm(&mut self, vm_function: &VMFunction, constant_map: &HashMap<Value, usize>) {
            ...
            for instruction in &function.instructions {
                let vm_instruction = match instruction {
                    ...
                    CompiledInstruction::JMPEQ(label) => {
                        Instruction::JMPEQ(label.clone())
                    },
                    CompiledInstruction::JMPNEQ(label) => {
                        Instruction::JMPNEQ(label.clone())
                    },
                    CompiledInstruction::JMPLT(label) => {
                        Instruction::JMPLT(label.clone())
                    },
                    CompiledInstruction::JMPGT(label) => {
                        Instruction::JMPGT(label.clone())
                    },
    ...
    fn serialize_value<W: Write>(&self, value: &Value, writer: &mut W) -> BinaryResult<()> {
        match value {
            ...
            _ => return Err(BinaryError::InvalidData(format!("Unsupported value type for serialization: {:?}", value))),
        }
    ...
    ///
    /// A result indicating success or an error
    fn serialize_function<W: Write>(&self, function: &CompiledFunction, writer: &mut W) -> BinaryResult<()> {
        // Write function name
    ...
    ///
    /// A result indicating success or an error
    fn serialize_instruction<W: Write>(&self, instruction: &CompiledInstruction, writer: &mut W) -> BinaryResult<()> {
        match instruction {
            CompiledInstruction::LD(reg, offset) => {
    ...
    fn deserialize_value<R: Read>(reader: &mut R) -> BinaryResult<Value> {
        ...
        match type_tag[0] {
            ...
            _ => Err(BinaryError::InvalidData(format!("Unknown value type tag: {}", type_tag[0]))),
        }
    }
    ...
    fn deserialize_function<R: Read>(reader: &mut R) -> BinaryResult<CompiledFunction> {
        ...
        for _ in 0..param_count {
            ...
            let param = String::from_utf8(param_bytes)
                .map_err(|_| BinaryError::InvalidData("Invalid UTF-8 in parameter name".to_string()))?;
        ...
        for _ in 0..upvalue_count {
            ...
            let upvalue = String::from_utf8(upvalue_bytes)
                .map_err(|_| BinaryError::InvalidData("Invalid UTF-8 in upvalue name".to_string()))?;
        ...
        let parent = if has_parent_byte[0] == 1 {
            ...
            let parent = String::from_utf8(parent_bytes)
                .map_err(|_| BinaryError::InvalidData("Invalid UTF-8 in parent function name".to_string()))?;
    ...
    fn deserialize_instruction<R: Read>(reader: &mut R) -> BinaryResult<CompiledInstruction> {
        ...
        match opcode {
            ...
            x if x == ReducedOpcode::CALL as u8 => {
                ...
                let mut name_bytes = vec![0u8; name_len];
                reader.read_exact(&mut name_bytes)?;
                let function_name = String::from_utf8(name_bytes)
                    .map_err(|_| BinaryError::InvalidData("Invalid UTF-8 in function name".to_string()))?;
            ...
            _ => Err(BinaryError::InvalidData(format!("Unknown opcode: {}", opcode))),
        }
    }
}
